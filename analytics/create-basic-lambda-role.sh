...
ROLE_NAME="BasicLambdaExecutionRole"
...
if [ ! -z "$EXISTING_ROLE" ] && [ "$EXISTING_ROLE" != "None" ]; then
    ...
    if [ ! -z "$POLICY_ATTACHED" ]; then
        ...
        echo "✅ AWSLambdaBasicExecutionRole policy attached!"
        echo "Role ARN: $EXISTING_ROLE"
        exit 0
    fi
fi
...
echo "✅ Basic Lambda execution role setup complete!"
echo "Role ARN: $ROLE_ARN"
echo ""
...
echo ""
echo "Role is ready to use in your CloudFormation templates."