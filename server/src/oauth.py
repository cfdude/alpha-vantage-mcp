...
def handle_metadata_discovery(event: dict) -> dict:
    ...
    if domain_name:
        ...
        if not host:
            return {"statusCode": 400, "body": json.dumps({"error": "missing_host_header"})}
...
def handle_authorization_form_submission(event: dict, query_params: dict) -> dict:
    ...
    return {
        "statusCode": 302,
        "headers": {"Location": redirect_url, "Cache-Control": "no-store", "Pragma": "no-cache"},
    }
...
def handle_authorization_code_grant(params: dict) -> dict:
    ...
    # Validate code data
    if code_data.get("client_id") != client_id or code_data.get("redirect_uri") != redirect_uri:
        return {
            "statusCode": 400,
            "body": json.dumps(
                {"error": "invalid_grant", "error_description": "Code validation failed"}
            ),
        }
    ...
    if not api_key:
        return {
            "statusCode": 400,
            "body": json.dumps(
                {"error": "invalid_grant", "error_description": "No API key found in authorization"}
            ),
        }
...
def create_error_redirect(redirect_uri: str, error: str, state: str | None = None) -> dict:
